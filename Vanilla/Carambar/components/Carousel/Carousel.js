...
class Carousel extends HTMLElement {
    ...
    connectedCallback() {     
        ...
        const template = currentDocument.querySelector('#carousel-component');                
    ...
    appear_video(number){
        for(let i = 0;i<3;i++){
            if (i === number){
                this.shadowRoot.getElementById("video" + i.toString()).style.display = "block";
            }else{
                this.shadowRoot.getElementById("video" + i.toString()).style.display = "none";
            }
        }
    }
}
